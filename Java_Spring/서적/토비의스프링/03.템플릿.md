...
Select 해온 결과를 requiredType 인자에서 지정된 클래스 타입으로 반환하는 템플릿입니다.

sql의 결과값은 single row, single colume이어야 합니다.
...
Select 해온 결과를 rowMapper 콜백에서 정의된 코드를 통해 `T` 타입으로 매핑한 후 반환합니다.

sql의 결과값은 single row여야 하며, multi colume이어도 됩니다.

RowMapper는 ResultSet을 받아,  `mapRow` 메소드에서 ResultSet의 결과를 추출하고 매핑하는 역할을 수행합니다. 
...
sql의 결과값은 multi row, multi colume이어도 됩니다. RowMapper는 마찬가지로 ResultSet을 받아 `mapRow` 메소드에서 ResultSet의 결과를 추출하고 매핑하는 역할을 수행합니다.  대신, ResultSet의 모든 로우를 열람하면서 로우마다 RowMapper 콜백을 호출하게 됩니다.
