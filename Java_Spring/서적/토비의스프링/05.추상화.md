...
세번째는 Connection 객체의 사용으로 **특정 데이터 엑세스 기술에 종속적**이게 된다. JPA / Hibernate의 경우에는 Connection 객체가 아닌 EntityManager 혹은 Session 필요로 한다. DAO 메소드에 Connection 파라미터를 추가하게 되면, DAO Interface는 DAO별로 사용하는 데이터 엑세스 기술마다 Interface를 별도로 생성해야 되는 것이다. 이렇게 된다면 더 이상 Interface를 통한 느슨한 관계 형성도 의미가 없어질 것이다. 또한 Service에서도 Connection 
