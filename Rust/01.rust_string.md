...
문자열은 할당받은 메모리 크기의 재조정이 가능하다. 왜냐하면 String은 여타 primitive 타입과 다르게 필요한 메모리 크기가 고정되어 있지 않다. 선언 당시의 문자열에서 점점 문자열의 길이가 길어진다면 더 큰 크기의 메모리 공간으로 재배정을 받게 된다.
...
Rust의 모든 문자열 타입은 유효한 UTF-8(valid UTF-8) 이 보장된다. 또한, 문자열을 구성하는 배열은 char가 아닌  Unicode 코드로 이루어진다. 
...
우선, `String`은 owned string이 필요할 때 사용된다. 예를 들어, 다른 thread로 보낸다던가, owned string을 가지는 struct를 생성하는 경우가 해당된다. 새로운 `String`을 받는 작업은 항상 메모리 할당을 유발한다. 메모리 재할당은 비싼 작업이기 때문에 꼭 필요한 경우가 아니라면 새로운 String을 사용하지 말아야 한다. 한 예로, loop를 돌면서 한 String의 다른 부분들에 접근해서 읽는다고 해보자. String 조각을 String 타입으로 받게되면 계속해서 메모리 할당이 발생할 것이고, 이는 비효율을 초래한다.



`&String`은 주로 메소드의 파라미터로 문자열을 넘길 때, 소유권을 원래 객체가 유지해야 하는 경우 사용된다. 즉, 메소드 파라미터에 소유권 이전 없이, 참조만 넘길 때 사용된다.  참조를 넘기더라도, `mut &String` 타입으로 실제 값에 대한 수정 또한 가능하다.

:link: [[Rust] 소유권](https://dev-ote.tistory.com/5)
...
참고로, 메소드에서 정의된 파라미터 타입은 `String` 이지만, 전달인자로는 `&String`나 `&str` 타입을 넣는 경우에도 메소드 내에서 메모리 할당이 발생한다. 그렇기에, 이 경우에는 처음부터 `String` 타입으로 메소드에 전달하는 것이 좋을 것이다.



`&str`은 실제 값을 읽기만 할 때 사용된다. 아까와 같은 loop를 돌며 문자열을 읽는 경우에도, `String`이 아닌 `&str`를 사용한다면 추가적인 메모리 할당 없이 작업 수행이 가능하다. 

또한, `&str`은 `&String`을 대신하여 변수를 받을 수 있다 (역방향은 성립하지 않는다).  `&str`으로 `String` 값에 대한 참조가 가능하며, `&String`을 `&str`으로 전환하는 것은 값싼 작업에 속한다. 따라서, 함수에서 소유권과 수정이 필요 없다면, 가장 효율적이다. (추가적으로, 소유권 이전과 수정이 발생하지 않게 강제할 수 있다)
