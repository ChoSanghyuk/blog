...
let s = ['h', 'e', 'l', 'l', 'o'];
let size: usize = s.into_iter().map(|c| mem::size_of_val(&c)).sum(); // 배열 요소를 iterate하며 메모리 합함
assert_eq!(size, 20);
```
...
&str
 ...
 - 실제 값을 읽기만 할 때 사용
 - 아까와 같은 loop의 경우에도, String이 아닌 &str를 사용한다면 추가적인 메모리 할당 없이 작업 수행이 가능핟.
 - 또한, &str은 &String을 대신하여 변수를 받을 수 있다 (&String can be deref-coerced to &str). &str로의 conversion은 inexpensive 하지만, 역방향으로는 성립 x
이 작업은 
