# ERC-20 vs ERC-1400 vs ERC-3643



ERC-1400

 이더리움의 증권형 토큰의 발행, 유통, 소유권, transfer 제한, 권리 등에 대한 표준 정립을 위해 제안된 토큰 표준이며, ERC-20이나 ERC-777과 같은 기존 토큰 표준들과 호환성을 가지고 있습니다. ERC-1400은 아래 그림과 같이 ERC-1410, ERC-1594, ERC-1643, ERC-1655 모두를 통합하여 활용한 Umbrella Architecture 형태로 구현되었습니다.
...
:bulb: SSI : https://web3explorer.medium.com/%EC%9E%90%EA%B8%B0-%EC%A3%BC%EA%B6%8C-%EC%8B%A0%EC%9B%90%EA%B3%BC-%EA%B5%AC%EC%84%B1-%EC%9A%94%EC%86%8C-self-sovereign-identity-452df0e36936
...
:bulb: permissioned tokens : blockchain-based tokens that include access controls or restrictions on who can hold, transfer, or interact with them
...
need to deploy **two main contracts**:

**Compliance Token Contract**: This is the core contract that represents the security token itself. It adheres to ERC-3643 standards, ensuring compliance with the required functionalities for identity, permissions, and transfer restrictions based on compliance rules. This contract also includes features from the ERC-20 and ERC-721 standards, as ERC-3643 is compatible with both.

**Identity Registry Contract**: This contract manages the identities of token holders, ensuring only compliant, verified participants can hold or transfer the security tokens. The identity registry keeps a list of approved investors and enforces compliance by integrating with the token contract, typically using a whitelist to manage the list of allowed wallet addresses.

**Optional and Supporting Contracts**

While the two contracts above are essential, you may also deploy additional optional contracts depending on the complexity of your setup:

- **Compliance Registry Contract**: If your system requires multiple compliance rules (for example, for different jurisdictions or investor types), a separate compliance registry can define and manage these rules dynamically.
- **Modular Components**: Some implementations use modular or pluggable components for flexibility, allowing for upgrades to compliance logic or identity verification without redeploying the core token contract.
